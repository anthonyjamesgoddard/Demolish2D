...
void Scenario::step() {
    ...
    for (int i = 0; i < numberOfObjects; i++) {
        ...
        for (int j = i + 1; j < numberOfObjects; j++) {
            ...
            float DBLsquared =
                (std::get<0>(locationi) - std::get<0>(locationj)) *
                    (std::get<0>(locationi) - std::get<0>(locationj)) +
                (std::get<1>(locationi) - std::get<1>(locationj)) *
                    (std::get<1>(locationi) - std::get<1>(locationj));
        ...
        if (CHbreach == true) {
            ...
            for (int i = 0; i < A.size(); i++) {
                ...
                for (int j = 0; j < B.size(); j++) {
                    ...
                    auto minparams = minimumDistanceBetweenLineSegments(
                        pi1, pi2, pj1, pj2, 100, 0.001);
                    auto minimumDistanceVertices =
                        verticesOnLineSegments(pi1, pi2, pj1, pj2, minparams);
                    float distance = calculateDistanceBetweenVertices(
                        minimumDistanceVertices);
                    if (distance < 0.01) {
                        std::array<float, 4> thetaRanges = {
                            pi1.getTheta(), pi2.getTheta(), pj1.getTheta(),
                            pj2.getTheta()};

                        // POUND-BACK BREACHED SECTORS
                        std::pair<int, int> sectorPair =
                            std::make_pair(pi1.getAssociatedSectorIndex(),
    ...
    for (auto& bsecs : _breachedSectors) {
        ...
        for (int m = 0; m < aNumberWeNeedToActuallyCorrectlyDefine; m++) {
            auto Asector = _objects[bsecs.first.first]
                               ._sectors[bsecs.second.first]
                               .obtainCurrentLevelOfDetailInWorld(locOfA);
            auto Bsector = _objects[bsecs.first.second]
                               ._sectors[bsecs.second.second]
                               .obtainCurrentLevelOfDetailInWorld(locOfB);
            for (int i = 0; i < Asector.size() - 1; i++) {
                ...
                for (int j = 0; j < Bsector.size() - 1; j++) {
                    ...
                    float distance = calculateDistanceBetweenVertices(
                        minimumDistanceVertices);
                    if (distance < 0.01) {
                        result1 = _objects[bsecs.first.second]
                                      ._sectors[bsecs.second.second]
                                      .generateNextLoD();
                        result2 = _objects[bsecs.first.first]
                                      ._sectors[bsecs.second.first]
                                      .generateNextLoD();
        ...
        for (int i = 0; i < Asector.size() - 1; i++) {
            ...
            for (int j = 0; j < Bsector.size() - 1; j++) {
                ...
                auto minimumDistanceVertices =
                    verticesOnLineSegments(pi1, pi2, pj1, pj2, minparams);
                float distance =
                    calculateDistanceBetweenVertices(minimumDistanceVertices);
                if (distance < 0.01) {
                    _collisonPoints.push_back(
                        std::get<0>(minimumDistanceVertices));
                    _collisonPoints.push_back(
                        std::get<1>(minimumDistanceVertices));
                }
            }
        }
    }
